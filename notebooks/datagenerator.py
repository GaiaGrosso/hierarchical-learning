...
class jet_data_generator(object):
    ...
    """
    def __init__(self, massprior, quarkmass, quarkp, nprong, nparticle, doFixP, doMultiprocess=False, ncore = 0):
    ...
    def hardsplit(self, mother, nthsplit):
        ...
        if nthsplit == 1 and self.massprior == "signal": # top decay
            dau1_m = 80.379 # W boson 
            dau2_m = 4.18 # b quark
            
        if nthsplit == 2 and self.massprior == "signal": # Z decay
            dau1_m = 4.18 # b quark
            dau2_m = 4.18 # b quark
    ...
    def draw_first_particle(self):
        ...
        if self.massprior == "signal":
            m = np.random.normal(172.76, 0.3) # top quark

        if self.massprior == "background":
            m = np.random.uniform(0, self.quarkmass*2)
        
        p = np.random.exponential(self.quarkp)
        #delete later
        if self.doFixP:
            p = self.quarkp
